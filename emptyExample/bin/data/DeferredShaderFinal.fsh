...
void main()
{
  ...
  vec3 fres = normalize(texture2D(AONoiseTexture,uv_var*vec2(15.0,8.4375)).xyz);
   ...
   for(int i=0; i<10;++i)
   {
      ...
      occluderFragment = texture2D(depthTexture,uv_var + sign(dot(ray,normal) )*ray.xy);
    // if depthDifference is negative = occluder is behind current fragment
      depthDifference = depthScreen-occluderFragment.r;
 
      // calculate the difference between the normals as a weight
 // the falloff equation, starts at falloff and is kind of 1/x^2 falling
      bl += step(falloff,depthDifference)*(1.0-dot(occluderFragment.xyz,normal))*(1.0-smoothstep(falloff,strength,depthDifference));
   }
   bl/=10.0;
    bl =1.0-bl;
